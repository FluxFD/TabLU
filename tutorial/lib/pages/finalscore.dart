...
class EventData {
  ...
  factory EventData.fromJson(Map<String, dynamic> json) {
    return EventData(
      ...
      criterias: List<Criteria>.from(json['criterias'].map((criteria) => Criteria.fromJson(criteria))),
      judges: List<aJudge>.from(json['judges'].map((judge) => aJudge.fromJson(judge))),
    );
  }
}
...
class aJudge {
  ...
  factory aJudge.fromJson(Map<String, dynamic> json) {
    return aJudge(
      ...
      contestants: List<Contestant>.from(json['contestants'].map((contestant) => Contestant.fromJson(contestant))),
    );
  }
}
...
class _WinnerState extends State<Winner> {
  ...
  Future<String> generatePdf(List<ScoreCard> scoreCards, EventData eventData) async {
    ...
    for (int i = 0; i < scoreCards.length; i++) {
      ...
      String line = '${ranking.isNotEmpty ? "$ranking - " : ""}${scoreCard.contestantName}: ${scoreCard.score.toStringAsFixed(2)}%';
      graphics.drawString(line, contentFont, brush: PdfBrushes.black, bounds: Rect.fromLTWH(0, contentYPosition, page.getClientSize().width, 20));
